...
def render_frame(bg_path: Path) -> Image.Image:
    ...
    h1 = text_h(_visual_hebrew("כתבו לי"), font_small)
    h2 = text_h("CLUB", font_large)
    h3 = text_h(_visual_hebrew("לניתוח המלא"), font_medium)
    total_h = h1 + LINE_GAP + h2 + LINE_GAP + h3

    y = TEXT_CENTER_Y - total_h // 2

    y = _draw_centered_text(draw, "כתבו לי", font_small, y, WHITE)
    y += LINE_GAP
    y = _draw_centered_text(draw, "CLUB", font_large, y, WHITE)
    y += LINE_GAP
    _draw_centered_text(draw, "לניתוח המלא", font_medium, y, WHITE_DIM)
